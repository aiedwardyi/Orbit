...
# Package upgrades can preserve application ancestors as 0775, which makes the
# bundled Cua Driver correctly reject its own executable path. A configured DEB
# also needs Electron's Chromium sandbox to be root-owned and setuid. Repair
...
if [ -n "${OPENMAUSBOT_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  case "$TEST_ROOT" in
    ...
    *)
      echo "Orbit test install root must stay under /tmp" >&2
      exit 1
      ;;
  esac
...
repair_directory() {
  ...
  if [ -L "$target" ] || [ ! -d "$target" ]; then
    echo "Orbit package directory is missing or unsafe: $target" >&2
    exit 1
  fi
  ...
  if [ "$actual" != "$EXPECTED_OWNER:755" ]; then
    echo "Orbit could not secure package directory: $target ($actual)" >&2
    exit 1
  fi
}
...
repair_executable() {
  ...
  if [ -L "$target" ] || [ ! -f "$target" ]; then
    echo "Orbit package executable is missing or unsafe: $target" >&2
    exit 1
  fi
  if [ "$TEST_MODE" -eq 0 ]; then chown root:root -- "$target"; fi
  chmod 0755 -- "$target"
  actual="$(stat -c '%U:%G:%a' -- "$target")"
  if [ "$actual" != "$EXPECTED_OWNER:755" ]; then
    echo "Orbit could not secure package executable: $target ($actual)" >&2
    exit 1
  fi
}
...
repair_chromium_sandbox() {
  target=$1
  if [ -L "$target" ] || [ ! -f "$target" ]; then
    echo "Orbit Chromium sandbox is missing or unsafe: $target" >&2
    exit 1
  fi
  if [ "$TEST_MODE" -eq 0 ]; then chown root:root -- "$target"; fi
  chmod 4755 -- "$target"
  ...
  if [ "$actual" != "$EXPECTED_OWNER:4755" ]; then
    echo "Orbit could not secure Chromium sandbox: $target ($actual)" >&2
    exit 1
  fi
}
