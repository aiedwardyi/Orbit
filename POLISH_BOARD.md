...
| ORCH-23a | Long tasks have no provider-neutral resume record before transcript truncation or session loss | Atlas | `task-state.test.ts`, `task-state-fold.test.ts`, `server/index.test.ts` continuity integration, `turn-context.test.ts`; full suite 2,323 passed | Persist a redacted, bounded goal, plan, progress, evidence, artifacts, blockers, and next action record and inject it into recovery turns | CLOSED |
...
| Full test suite | 2,323 passed, 99 skipped |
