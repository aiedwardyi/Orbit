...
Meta Muse runs as a Linux process behind the `wsl` wrapper. Only `META_API_KEY` crosses that boundary (via `WSLENV`); MCP server environment beyond it (`ELECTRON_RUN_AS_NODE`, `OMB_*` control variables) and non-command paths do not, so integrations that depend on them are unavailable to Meta turns on Windows. Session working directories and MCP server commands are translated to Linux paths. Normal turns without integrations are unaffected.
