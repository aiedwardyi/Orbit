...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "orbit-desktop"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing Orbit installation");
} catch (error) {
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: Orbit CI <ci@orbit.invalid>",
      "Description: Orbit directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
