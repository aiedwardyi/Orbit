...
export function TerminalWorkspace({ bot, onClose, focusBlocked, visible }: { bot: Pick<Bot, "id" | "name" | "busy">; onClose: () => void; focusBlocked: boolean; visible: boolean }) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    void bridge.open({ botId: bot.id, cols: terminal.cols, rows: terminal.rows, restart: generation > 0 }).then((result) => {
      if (!alive) return;
      id = result.id;
      lastSeq = result.seq;
      ...
      pending.length = 0;
      const code = exits.get(id) ?? result.exitCode;
      setExitCode(code);
      terminal.options.disableStdin = code !== null;
      setSession({ cwd: result.cwd, shell: result.shell });
      resize();
    ...
    <main className="flex min-h-0 min-w-0 flex-1 flex-col bg-inset text-ink" aria-label={t("terminal.title")}>
      <header className="flex min-h-[60px] shrink-0 items-center gap-3 border-b border-hairline bg-panel py-3 pl-11 pr-5 md:pl-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate font-mono text-[11px] text-ink-secondary" title={session?.cwd}>{session?.cwd ?? (error ? t("terminal.unavailable") : t("terminal.connecting"))}</p>
        </div>
        {bot.busy && <span className="shrink-0 text-[11px] text-accent-text" role="status">{t("terminal.agentWorking")}</span>}
      </header>
      {error && <div role="alert" className="flex items-center justify-between gap-3 border-b border-danger/30 bg-danger/10 px-5 py-3 text-[13px] text-danger">
        <span>{error}</span>
      ...
      <footer className="flex shrink-0 flex-wrap items-center justify-between gap-x-4 gap-y-1 border-t border-hairline bg-panel px-5 py-2 text-[11px] text-ink-secondary">
        <span>{exitCode === null ? session?.shell.split(/[\\/]/).at(-1) ?? (error ? t("terminal.unavailable") : t("terminal.connecting")) : t("terminal.exited", { code: exitCode })}</span>
        {exitCode !== null ? (
